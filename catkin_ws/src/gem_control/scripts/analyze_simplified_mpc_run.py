...
def analyze(run_directory: Path) -> dict[str, object]:
    ...
    summary = {
        ...
        "horizon_duration_s": controller_summary["horizon_duration_s"],
        ...
        "steering": {
            "maximum_absolute_published_command_rad": float(
                np.max(
                    np.abs(columns["published_steering_command_rad"])
                )
            ),
            "saturation_count": int(
                np.count_nonzero(
                    np.abs(columns["raw_steering_command_rad"]) > 0.5
                )
            ),
        },
    ...
    path_axis.set_title("Simplified-MPC path tracking")
    path_axis.legend()
    ...
    speed_axis.plot(
        time_s, columns["reference_speed_mps"], label="reference"
    )
    speed_axis.plot(
        time_s,
    ...
    timing_axis.plot(time_s, 1000.0 * compute, label="complete MPC")
    ...
    constraint_axis.set_title("Simplified-MPC cross-track error")
