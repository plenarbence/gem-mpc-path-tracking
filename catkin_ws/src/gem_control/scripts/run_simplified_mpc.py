...
class SimplifiedMpcExecutionNode:
    def __init__(self) -> None:
        self.config = load_simplified_mpc_config(
        ...
        self.output_directory = Path(
            rospy.get_param(
                "~output_directory",
                "/workspace/results/simplified_mpc/simulator_run",
            )
        )
        ...
        self.stamped_publisher = rospy.Publisher(
            rospy.get_param(
                ...
                "/gem_control/simplified_mpc/ackermann_cmd_stamped",
            ),
        ...
        rospy.loginfo(
            "Building horizon-%d simplified kinematic MPC",
            self.config.horizon_steps,
        )
        self.mpc = SimplifiedKinematicMpc(config=self.config)
        self.delay_model = LearnedDynamics(selected_model_directory())
        rospy.loginfo(
            "Simplified MPC built in %.3f s; learned model reserved for "
            "one delay-prediction step",
    ...
    def publish_reference_path(self) -> None:
        ...
        progress = np.linspace(0.0, self.mpc.reference_path.length, 1201)
    ...
    def reference_speed(self, elapsed_s: float) -> float:
        ...
        fraction = max(
            0.0,
            1.0
            - (elapsed_s - self.drive_duration_s)
            / self.stop_ramp_duration_s,
        )
    ...
    def run(self) -> None:
        ...
        for index in range(3):
            warmup = self.mpc.solve(
                predicted_vehicle_state=initial_state,
                previous_progress_m=initial_projection.s,
                reference_speed_mps=0.5,
                warm_start_allowed=index > 0,
                enforce_deadline=False,
            )
            if not warmup.diagnostics.solution_accepted:
                raise RuntimeError("stationary simplified-MPC warmup failed")
            warmup_times.append(warmup.diagnostics.solve_time_s)
        rospy.loginfo(
            "Stationary warmup complete: %s ms",
            ", ".join("{:.1f}".format(1000.0 * item) for item in warmup_times),
        )
        ...
        if target_progress_m is None:
            ...
            rospy.loginfo(
                "Simplified-MPC lap mode: %.3f laps, %.1f m target, "
                "%.1f s timeout",
                self.target_laps,
        ...
        while not rospy.is_shutdown():
            ...
            projection = self.mpc.reference_path.project_local(
                measured_state[0], measured_state[1], progress_hint
            )
            ...
            optimization_start = project_learned_start_state(
                delayed.predicted_state, self.config
            )
            target_speed = (
            ...
            result = self.mpc.solve(
                predicted_vehicle_state=optimization_start,
            ...
            rows.append(
                {
                    ...
                    "delay_prediction_steps": 1,
                    "delay_prediction_model": "learned_h2",
                    ...
                    "predicted_start_x_m": delayed.predicted_state[0],
                    "predicted_start_y_m": delayed.predicted_state[1],
                    "predicted_start_yaw_rad": delayed.predicted_state[2],
                    "predicted_start_speed_mps": delayed.predicted_state[3],
                    "projected_start_speed_mps": optimization_start[3],
                    "predicted_start_yaw_rate_radps": (
                        delayed.predicted_state[4]
                    ),
                    ...
                    "reference_speed_mps": target_speed,
                    "published_speed_command_mps": (
                        delayed.active_command[0]
                    ),
                    "published_steering_command_rad": (
                        delayed.active_command[1]
                    ),
                    "next_speed_command_mps": active_command[0],
                    "next_steering_command_rad": active_command[1],
                    "first_delta_speed_mps": (
                        result.first_delta_speed_mps
                    ),
                    "first_delta_yaw_rad": result.first_delta_yaw_rad,
                    "desired_next_speed_mps": (
                        result.desired_next_speed_mps
                    ),
                    "desired_next_yaw_rad": (
                        result.desired_next_yaw_rad
                    ),
                    "raw_steering_command_rad": (
                        result.raw_steering_command_rad
                    ),
                    "steering_command_saturated": (
                        result.steering_command_saturated
                    ),
                    "solver_success": result.success,
                    "solver_status": (
                        result.diagnostics.return_status
                    ),
                    "solver_iterations": (
                        result.diagnostics.iteration_count
                    ),
                    ...
                    "constraint_violation": (
                        result.diagnostics.maximum_dynamics_residual
                    ),
                    "objective": result.diagnostics.objective,
                    "warm_start_source": (
                        result.diagnostics.warm_start_source
                    ),
                    "fallback_reason": (
        ...
        rospy.loginfo(
            "Simplified-MPC run finished: %s, %d control cycles",
            termination_reason,
            len(rows),
        )
    ...
    def write_results(
        ...
        summary = {
            ...
            "horizon_duration_s": (
                self.config.horizon_steps * self.config.period_s
            ),
            ...
            "delay_prediction": {
                "steps": 1,
                "model": "learned_h2",
                "commissioned_delay_s": self._commissioned_delay_s,
            },
            ...
            "mean_total_compute_time_s": (
                float(np.mean(compute_times)) if len(compute_times) else None
            ),
            "p95_total_compute_time_s": (
                float(np.quantile(compute_times, 0.95))
                if len(compute_times)
                else None
            ),
            "maximum_total_compute_time_s": (
                float(np.max(compute_times)) if len(compute_times) else None
            ),
...
def main() -> None:
    rospy.init_node("simplified_mpc_controller")
    node: SimplifiedMpcExecutionNode | None = None
    try:
        node = SimplifiedMpcExecutionNode()
        node.run()
    except Exception as error:
        if node is not None:
            node.publish_stop()
        rospy.logfatal("Simplified MPC failed: %s", error)
        raise
